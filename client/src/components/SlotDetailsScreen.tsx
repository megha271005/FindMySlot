...
const SlotDetailsScreen = () => {
  ...
  const createBookingMutation = useMutation({
    mutationFn: async ({ locationId, slotId, duration }: { locationId: number; slotId: number; duration: number }) => {
      const res = await apiRequest("POST", "/api/bookings", { locationId, slotId, duration });
      return res.json();
    },
    onSuccess: (data) => {
      ...
      {isLoading ? (
        ...
        <>
          ...
          <div className="p-4">
            ...
            {/* Booking Options */}
            <h3 className="font-medium mb-3">Select Booking Duration</h3>
            <div className="grid grid-cols-3 gap-3 mb-6">
              <div 
                className={`border rounded-lg p-3 text-center cursor-pointer ${selectedDuration === BOOKING_DURATIONS.THIRTY_MIN ? 'border-primary bg-primary/10' : 'border-input'}`}
              ...
              <div 
                className={`border rounded-lg p-3 text-center cursor-pointer ${selectedDuration === BOOKING_DURATIONS.ONE_HOUR ? 'border-primary bg-primary/10' : 'border-input'}`}
              ...
              <div 
                className={`border rounded-lg p-3 text-center cursor-pointer ${selectedDuration === BOOKING_DURATIONS.TWO_HOURS ? 'border-primary bg-primary/10' : 'border-input'}`}
